...
from sitesetting.models import (Sites,Faq,AboutCompany)
# app import models
...
class PartnerWithUsView(FormView):
    ...
    success_url = reverse_lazy("partner_with_us")

    def form_valid(self, form):
        # Verify OTP if it was sent
        if 'otp' in self.request.session:
            user_otp = form.cleaned_data.get('otp', '')
            session_otp = str(self.request.session.get('otp', ''))
            otp_email = self.request.session.get('otp_email', '')
            ...
            if form.cleaned_data['email'] != otp_email:
                return JsonResponse({
                    'status': 'error',
                    'message': 'Email does not match the OTP recipient.',
                    'errors': {'email': ['Email does not match the OTP recipient.']}
                }, status=400)
            
            # Clear OTP data from session after successful verification
            del self.request.session['otp']
            del self.request.session['otp_time']
            del self.request.session['otp_email']
        
        # Save form data (in a real app, you'd save to database here)
        session_data = form.cleaned_data
        self.request.session['partner_form_data'] = json.dumps(session_data)
...
def send_otp(request):
    ...
    try:
        # Send email
        send_mail(
            'Your OTP for Partner Registration',
...
def robots_txt(request):
    content = """User-agent: *
        Disallow: /admin/
        Allow: /
        Sitemap: https://fixenix.com/sitemap.xml
        """
...
class OtpVerify(View):
    ...
    template_name = f"pages/otpverify.html"
    model = EstimateRequests

    def get(self, request, *args, **kwargs):
        # Check if required session data exists
        required_session_keys = ['otp', 'name', 'email', 'mobile', 'device_name', 'device_problem', 'time']
        if not all(key in request.session for key in required_session_keys):
            ...
            return redirect('otp_verify')  # Replace with your actual start view
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            return self.handle_valid_otp(form)
        return self.handle_invalid_otp(form)

    def handle_valid_otp(self, form):
        # Combine OTP digits
        otp_digits = [
            form.cleaned_data["otp1"],
            form.cleaned_data["otp2"],
            form.cleaned_data["otp3"],
            form.cleaned_data["otp4"]
        ]
        user_otp = int(''.join(map(str, otp_digits)))
        
        # Get session data
        session_otp = self.request.session.get("otp")
        expiry_time = self.request.session.get("time")
        ...
        return self.process_successful_verification()

    def process_successful_verification(self):
        # Create estimate request
        estimate = EstimateRequests.objects.create(
            name=self.request.session["name"],
            mobile=self.request.session["mobile"],
            email=self.request.session["email"],
            device_name=self.request.session["device_name"],
            device_problem=self.request.session["device_problem"],
        )

        # Send email notification
        self.send_notification_email(estimate)
        ...
        return JsonResponse({
            ...
            "redirect_url": reverse("index")  # Replace with your success URL
        })

    def send_notification_email(self, estimate):
        template_name = f"emails/jobsheet.html"
        context = {
            ...
            "device_problem": estimate.device_problem,
    ...
    def cleanup_session(self):
        keys_to_remove = [
            'otp', 'name', 'email', 'mobile',
            'device_name', 'device_problem', 'time'
        ]
        for key in keys_to_remove:
            if key in self.request.session:
                del self.request.session[key]

    def handle_invalid_otp(self, form):
        errors = {
            field: error[0] 
