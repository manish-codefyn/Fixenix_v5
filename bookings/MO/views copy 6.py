...
from .models import ( DoorstepService, Booking, Payment,RepairService,Review,
                     Device, DeviceBrand, DeviceModel, DeviceProblem,Delivery  )
...
def get_agent_location(request, delivery_id):
    try:
        delivery = Delivery.objects.get(id=delivery_id)
        return JsonResponse({
            "current_location_lat": delivery.current_location_lat,
            "current_location_lon": delivery.current_location_lon,
        })
...
@csrf_exempt
def update_agent_location(request, delivery_id):
    if request.method == "POST":
        try:
            delivery = Delivery.objects.get(id=delivery_id)
            data = json.loads(request.body)
            delivery.current_location_lat = data.get("current_location_lat")
            delivery.current_location_lon = data.get("current_location_lon")
            delivery.save()
...
def live_tracking_view(request, delivery_id):
    # Fetch delivery details using the delivery ID
    # Fetch delivery details
    delivery = Delivery.objects.get(id=delivery_id)
    context = {
        "current_location_lat": delivery.current_location_lat,
        "current_location_lon": delivery.current_location_lon,
        "delivery_location_lat": delivery.delivery_location_lat,
        "delivery_location_lon": delivery.delivery_location_lon,
        "delivery_id": delivery_id,
    }
    return render(request, "bookings/tracking.html", context)


def get_current_location(request, delivery_id):
    """API to fetch the delivery person's current location."""
    delivery = Delivery.objects.get(id=delivery_id)
    return JsonResponse({
        "lat": delivery.current_location_lat,
        "lon": delivery.current_location_lon,
    })



def update_delivery_status(request, delivery_id):
    """Update delivery status and trigger live tracking email if necessary."""
    delivery = Delivery.objects.get(id=delivery_id)
    status = request.POST.get("status", "")

    if status in ["out_for_delivery", "in_progress", "completed", "cancelled"]:
        delivery.status = status
        delivery.save()

        # Send live tracking email if status changes to out_for_delivery
        if status == "out_for_delivery":
            delivery.send_tracking_email()
...
def assign_job(request, employee_id, lat, lon):
    # Trigger the task to update the employee's location asynchronously
    update_employee_location.delay(employee_id, lat, lon)
...
class TrackBookingView(View):
    ...
    def get(self, request):
        form = BookingTrackForm()  # Instantiate an empty form
        return render(request, self.template_name, {'form': form})

    def post(self, request):
        form = BookingTrackForm(request.POST)  # Bind form with POST data
        context = {'form': form}

        if form.is_valid():
            booking_id = form.cleaned_data['booking_id']
            try:
                # Attempt to fetch the booking by ID
                booking = Booking.objects.get(booking_id=booking_id)
                
                # Try to get the associated delivery, using filter().first() to avoid errors
                delivery = Delivery.objects.filter(booking=booking).first()

                if delivery:  # If a matching Delivery is found
                    context['delivery'] = delivery
                else:  # If no matching delivery is found
                    messages.error(request, f"No delivery found for booking ID {booking_id}.")
                    context['error'] = f"No delivery found for booking ID {booking_id}."

                context['booking'] = booking  # Add booking to context

            except Booking.DoesNotExist:
                messages.error(request, f"No booking found with ID {booking_id}.")
                context['error'] = f"No booking found with ID {booking_id}."
        
        else:
            messages.error(request, "Please enter a valid booking ID.")
            context['error'] = "Please provide a valid booking ID."
...
#                 # Attempt to fetch the booking by ID
#                 booking = Booking.objects.get(booking_id=booking_id)
#                 delivery = Delivery.objects.get(booking=booking)  # Assuming a relationship with Booking
#                 context['booking'] = booking  # Add booking to context if found
#                 context['delivery'] = delivery  # Add booking to context if found
#             except Booking.DoesNotExist:
#                 # Add error message to context if booking not found
#                 messages.error(request, f"No booking found with ID {booking_id}.")
#                 context['error'] = f"No booking found with ID {booking_id}."
#         else:
#             messages.error(request, "Please enter a valid booking ID.")
    ...
    def post(self, request):
        ...
        try:
            ...
            if generated_signature == razorpay_signature:
                ...
                context = {"amount": payment.amount_paid}
