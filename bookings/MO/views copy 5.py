...
from .forms import RepairServiceForm, OTPVerificationForm,RepairStatusForm,BookingForm,ReviewForm,RepairServiceForm, OTPVerificationForm
...
class CalculateDistanceView(View):
    def post(self, request):
        try:
            ...
            # Example fixed service location (latitude, longitude)
            service_location = (26.747917, 88.437392)  # Example: Delhi, India
            user_location = (float(user_lat), float(user_lng))

            # Calculate distance
            distance = geodesic(service_location, user_location).kilometers
            return JsonResponse({"distance": round(distance, 2)})  # Round to 2 decimal places
